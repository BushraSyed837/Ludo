   ...
   for(i=0;i15;i++)
    {
        if(i=6&&i=9)
          continue;
           line(20+i30,200,20+i30,290);
    }
    for(i=0;i15;i++)
    {
        if(i=6&&i=9)
        continue;
      line(200,20+i30,290,20+i30);
=======
   for(i=0;i<15;i++)
    {
        if(i>=6&&i<=9)
...
<<<<<<< HEAD
    struct user  student;
     ...
     int num;
     printf(n HOW MANY PLAYERS  );
     scanf(%d,&n);
     student = (struct user )malloc(nsizeof(struct user));
      intialise the users
     for(int i =0;i n;i++)
     {
...
<<<<<<< HEAD
                      printf(  m %d  ,m);
                      for( m=0 ; m4;m++)   get the particular player
...
<<<<<<< HEAD
                   ...
                   printf(sum %dn,student[i].g[run].sum);
                     if( student[i].g[run].sum 50)
                     {    printf(  t t HOME TIME );

                          student[i].g[run].pos= home(color_g[i],first,first+num);
                               if(  student[i].g[run].pos == 6)
                                    {coutttt FINISHEDendl;   student[i].g[run].status=0;
                                        student[i].win+=1;
                                        if(student[i].win==4)
                                         {  coutttttWINNER IS i  endl;
=======
                      for( l=0;l<n;l++)
                          { if(color_g[l] == getcl)
                             break;
